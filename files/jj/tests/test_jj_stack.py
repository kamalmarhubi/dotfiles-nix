...
def test_revision_details_reads_one_batched_jsonl_query(monkeypatch):
    script = load_script()
    monkeypatch.setattr(
        script,
        "jj_inspect",
        lambda *_args, **_kwargs: subprocess.CompletedProcess(
            [], 0, '"push-" ++ change_id.short()\n', ""
        ),
    )
    records = [
        {
            ...
            "local_bookmarks": ["local-second"],
            "remote_bookmarks": ["remote-second"],
            "local_conflicts": [],
            "remote_conflicts": [],
            "description": "Second title\n\nSecond body\n",
            "generated_bookmark": "push-second",
        },
        {
            "commit_id": "first",
            "change_id": "change-first",
            "local_bookmarks": ["local-first"],
            "remote_bookmarks": [],
            "local_conflicts": [],
            "remote_conflicts": [],
            "description": "First title\n",
            "generated_bookmark": "push-first",
        },
    ]
    calls = []
    ...
    def output(*args):
        calls.append(args)
        return "".join(script.json.dumps(record) + "\n" for record in records)

    monkeypatch.setattr(script, "jj_output", output)

    details = script.revision_details(["first", "second"], "origin")

    assert len(calls) == 1
    assert calls[0][calls[0].index("-r") + 1] == "(first | second)"
    assert [detail.commit_id for detail in details] == ["first", "second"]
    assert details[0].local_bookmarks == ("local-first",)
    assert details[1].remote_bookmarks == ("remote-second",)
    assert details[1].body == "Second body"
    assert "generated_bookmark" in calls[0][-1]
...
def test_revision_details_rejects_incomplete_or_malformed_batch(monkeypatch, records):
    script = load_script()
    monkeypatch.setattr(
        script,
        "jj_inspect",
    ...
    monkeypatch.setattr(
        script,
        "jj_output",
        lambda *_args: "".join(script.json.dumps(record) + "\n" for record in records),
    )

    with pytest.raises(script.Error, match="revision metadata"):
        script.revision_details(["first"], "origin")
...
def pull_request_node(
    ...
    *,
    ...
    return {
        ...
        "headRefName": head,
        "headRefOid": head,
        "headRef": {"name": head, "target": {"oid": head}},
        "baseRefName": "main",
        "baseRefOid": "base",
...
def test_discovers_merged_selector_by_historical_oid_not_live_head(monkeypatch):
    ...
    selectors, base_oid = script.discover_merged_selectors(
        "owner/repo", ["old-head"], "main", {}, "."
    )

    assert base_oid == "base"
...
def test_rejects_oid_that_is_both_open_boundary_and_merged_selector(monkeypatch):
    ...
    with pytest.raises(script.Error, match="ambiguously both"):
        script.discover_merged_selectors("owner/repo", ["same"], "main", {}, ".")


def test_merged_selector_lookup_rejects_missing_or_moving_base(monkeypatch):
    script = load_script()
    monkeypatch.setattr(script, "_GRAPHQL_BATCH_SIZE", 1)

    monkeypatch.setattr(
        script,
        "graphql",
        lambda *_args: {"repository": {"baseRef": None, "commit0": None}},
    )
    with pytest.raises(script.Error, match="remote branch does not exist: main"):
        script.discover_merged_selectors("owner/repo", ["one"], "main", {}, ".")

    responses = iter(
        [
            {"repository": {"baseRef": {"target": {"oid": "base-a"}}, "commit0": None}},
            {"repository": {"baseRef": {"target": {"oid": "base-b"}}, "commit0": None}},
        ]
    )
    monkeypatch.setattr(script, "graphql", lambda *_args: next(responses))
    with pytest.raises(script.Error, match="base branch changed during discovery"):
        script.discover_merged_selectors("owner/repo", ["one", "two"], "main", {}, ".")


def test_merged_selector_lookup_batches_and_chunks(monkeypatch):
    script = load_script()
    monkeypatch.setattr(script, "_GRAPHQL_BATCH_SIZE", 2)
    calls = []
    ...
    def graphql(_query, variables, *_args):
        ...
        repository = {
            f"commit{index}": {
                "associatedPullRequests": {
                    "nodes": [
                        pull_request_node(f"live-{oid}", index + 1, historical_oid=oid)
                    ],
                    "pageInfo": {"hasNextPage": False},
                }
            }
            for index, oid in enumerate(
                value for key, value in variables.items() if key.startswith("oid")
            )
        }
        repository["baseRef"] = {"target": {"oid": "base"}}
        return {"repository": repository}

    monkeypatch.setattr(script, "graphql", graphql)

    selectors, base_oid = script.discover_merged_selectors(
        "owner/repo", ["one", "two", "three", "two"], "main", {}, "."
    )

    assert [
        [value for key, value in call.items() if key.startswith("oid")]
        for call in calls
    ] == [["one", "two"], ["three"]]
    assert base_oid == "base"
    assert list(selectors) == ["one", "two", "three"]


def test_open_pr_lookup_batches_and_chunks(monkeypatch):
    script = load_script()
    monkeypatch.setattr(script, "_GRAPHQL_BATCH_SIZE", 2)
    calls = []
    ...
    def graphql(_query, variables, *_args):
        ...
        repository = {
            ...
            for index, bookmark in enumerate(
                value for key, value in variables.items() if key.startswith("head")
            )
        }
        for index, bookmark in enumerate(
            value for key, value in variables.items() if key.startswith("head")
        ):
            repository[f"ref{index}"] = {"target": {"oid": bookmark}}
        return {"repository": repository}

    monkeypatch.setattr(script, "graphql", graphql)

    prs, refs = script.discover_pull_requests(
        "owner/repo", ["one", "two", "three", "two"], {}, "."
    )

    assert [
        [value for key, value in call.items() if key.startswith("head")]
        for call in calls
    ] == [["one", "two"], ["three"]]
    assert list(prs) == ["one", "two", "three"]
    assert refs == {"one": "one", "two": "two", "three": "three"}


def test_open_pr_lookup_rejects_deleted_branch(monkeypatch):
    script = load_script()
    ...
    with pytest.raises(script.Error, match="branch changed during discovery"):
        script.discover_pull_requests("owner/repo", ["topic"], {}, ".")
...
def test_lagging_pr_base_oid_does_not_trigger_boundary_refresh():
    script = load_script()
    merged = pull_request(
        script,
        "merged",
...
def test_queue_removal_uses_lowest_affected_stack_member():
    script = load_script()
    a = pull_request(script, "a", 1)
    b = pull_request(script, "b", 2, base="a", base_oid="a")
    c = pull_request(script, "c", 3, base="b", base_oid="b")
    queued = [
        script.PullRequest(**{**pr.__dict__, "merge_queue": True}) for pr in (a, b, c)
    ]
    stack = script.StackSnapshot("STACK_7", 7, queued, {pr.head: pr for pr in queued})
    plan = script.Plan(
    ...
    removal = script.plan_queue_removal(
        plan, stack, {"a": "a", "b": "new-b", "c": "c"}, {"b"}
    )

    assert [pr.number for pr in removal.affected] == [2, 3]
    assert [pr.number for pr in removal.targets] == [2]
    assert [pr.number for pr in removal.expected_removed] == [2, 3]
    assert [pr.number for pr in removal.expected_retained] == [1]
    assert [(cause.pr.number, cause.proposed_oid) for cause in removal.causes] == [
        (2, "new-b")
    ]
    assert script.plan_queue_removal(plan, stack, {}, set()) is None


def test_queue_removal_treats_unstacked_prs_independently():
    script = load_script()
    first = pull_request(script, "first", 1, stack_id=None, stack_number=None)
    second = pull_request(script, "second", 2, stack_id=None, stack_number=None)
    queued = [
        script.PullRequest(**{**pr.__dict__, "merge_queue": True})
        for pr in (first, second)
    ]
    stack = script.StackSnapshot(None, None, [], {pr.head: pr for pr in queued})
    plan = script.Plan(
    ...
    removal = script.plan_queue_removal(
        plan, stack, {"first": "first", "second": "second"}, set()
    )

    assert [pr.number for pr in removal.targets] == [1, 2]
    assert removal.expected_removed == removal.targets
    assert removal.expected_retained == ()


def test_queue_removal_is_manual_and_nonmutating(monkeypatch, capsys):
    script = load_script()
    a = script.PullRequest(
        **{**pull_request(script, "a", 1).__dict__, "merge_queue": True}
    )
    b = script.PullRequest(
        **{**pull_request(script, "b", 2).__dict__, "merge_queue": True}
    )
    c = script.PullRequest(
        **{**pull_request(script, "c", 3).__dict__, "merge_queue": True}
    )
    removal = script.QueueRemoval(
        (script.QueueCause(b, b.head_oid),), (b,), (b,), (b, c), (a,)
    )
    monkeypatch.setattr(
        script.sys, "stdin", type("Stdin", (), {"isatty": lambda _self: True})()
    )
    monkeypatch.setattr(script, "print_queue_cause", lambda *_args: None)
    monkeypatch.setattr(
        "builtins.input", lambda: pytest.fail("queue removal must not prompt")
    )

    with pytest.raises(script.Error, match="must be removed manually"):
        script.handle_queue_removal(removal, "/git", plan_only=False)

    output = capsys.readouterr().err
    assert f"Open PR #2 and choose Remove from queue: {b.url}" in output
    assert "also remove queued PR #3 above it" in output
    assert "Queued PRs below it should remain queued: #1" in output
    assert "Remove PR #2 from the merge queue now and stop? [y/N]" not in output


def test_independent_queue_removal_is_also_manual(monkeypatch, capsys):
    script = load_script()
    pr = pull_request(script, "topic", 1, stack_id=None, stack_number=None)
    pr = script.PullRequest(**{**pr.__dict__, "merge_queue": True})
    removal = script.QueueRemoval(
        (script.QueueCause(pr, pr.head_oid),), (pr,), (pr,), (pr,), ()
    )
    monkeypatch.setattr(
        script.sys, "stdin", type("Stdin", (), {"isatty": lambda _self: True})()
    )
    ...
    assert (
        f"Open PR #1 and choose Remove from queue: {pr.url}" in capsys.readouterr().err
    )


def test_plan_only_queue_removal_is_nonmutating(monkeypatch):
    script = load_script()
    pr = script.PullRequest(
        **{**pull_request(script, "topic", 1).__dict__, "merge_queue": True}
    )
    removal = script.QueueRemoval(
        (script.QueueCause(pr, pr.head_oid),), (pr,), (pr,), (pr,), ()
    )
    ...
    with pytest.raises(script.Error, match="no changes made"):
        script.handle_queue_removal(removal, "/git", plan_only=True)


def test_queue_cause_prints_description_diff_and_code_stat(monkeypatch, capsys):
    script = load_script()
    pr = pull_request(script, "topic", 1, head_oid="queued")
    descriptions = {"queued": "Old title\n\nBody", "local": "New title\n\nBody"}
    monkeypatch.setattr(
        script,
        "git_commit_description",
        lambda _root, oid: descriptions[oid],
    )
    monkeypatch.setattr(
        script,
        "command",
        lambda args, **_kwargs: script.subprocess.CompletedProcess(
            args,
            0,
    ...
    script.print_queue_cause(script.QueueCause(pr, "local"), "/git")

    output = capsys.readouterr().err
    assert "Commit description:" in output
    assert "-Old title" in output
    assert "+New title" in output
    assert "file.py | 2 +-" in output


def test_queue_cause_explicitly_prints_unchanged(monkeypatch, capsys):
    script = load_script()
    pr = pull_request(script, "topic", 1, head_oid="queued")
    monkeypatch.setattr(
        script, "git_commit_description", lambda _root, _oid: "Same title"
    )
    monkeypatch.setattr(
        script,
        "command",
        lambda args, **_kwargs: script.subprocess.CompletedProcess(args, 0, "", ""),
    )

    script.print_queue_cause(script.QueueCause(pr, "local"), "/git")

    output = capsys.readouterr().err
    assert "Commit description: unchanged" in output
    assert "Code: unchanged" in output
...
def test_default_plan_is_concise_and_verbose_plan_adds_evidence(capsys):
    ...
    script.print_plan(
        ...
        repo,
        "origin",
        script.Selection(("local-oid",), ("local-oid",), "old-base", "local-oid"),
        "base-oid",
        {},
        reconciliation=script.ReconciliationKind.ALREADY_CONVERGED,
        states=(state,),
    )
    stale = capsys.readouterr().err
    assert (
        "\nHint: stack base is behind origin/main; use --rebase to refresh it.\n"
        in stale
    )
    assert "  Stack base is behind" not in stale
...
def test_rejects_concurrent_stack_membership_or_order_change():
    ...
    edited_body = script.PullRequest(**{**a.__dict__, "body": "concurrent edit"})
    with pytest.raises(script.Error, match="membership, order, state, or refs changed"):
        script.verify_stack_snapshot_unchanged(
            before, script.StackSnapshot("STACK_7", 7, [edited_body, b])
        )
    ...
    lagging_a = script.PullRequest(
        **{**rewritten_a.__dict__, "base_oid": "lagging-main"}
    )
    lagging_b = script.PullRequest(**{**rewritten_b.__dict__, "base_oid": "lagging-a"})
    script.verify_stack_snapshot_unchanged(
        before,
        script.StackSnapshot("STACK_7", 7, [lagging_a, lagging_b]),
        {"main": "new-main", "a": "new-a", "b": "new-b"},
        {"main": "new-main", "a": "new-a", "b": "new-b"},
        normalize_after=True,
    )
    queued_lagging_a = script.PullRequest(**{**lagging_a.__dict__, "merge_queue": True})
    with pytest.raises(script.Error, match="membership, order, state, or refs changed"):
        script.verify_stack_snapshot_unchanged(
            before,
            script.StackSnapshot("STACK_7", 7, [queued_lagging_a, lagging_b]),
            {"main": "new-main", "a": "new-a", "b": "new-b"},
            {"main": "new-main", "a": "new-a", "b": "new-b"},
            normalize_after=True,
        )
...
def test_terminal_hyperlink_preserves_url_when_not_interactive(monkeypatch):
    script = load_script()
    monkeypatch.setattr(script.sys.stdout, "isatty", lambda: False)

    assert script.terminal_hyperlink(
        "#7 Subject\nline", "https://example.test/pr/7"
    ) == ("#7 Subject line (https://example.test/pr/7)")


def test_terminal_hyperlink_wraps_subject_when_interactive(monkeypatch):
    script = load_script()
    monkeypatch.setattr(script.sys.stdout, "isatty", lambda: True)

    assert script.terminal_hyperlink("#7 Subject", "https://example.test/pr/7") == (
        "\x1b]8;;https://example.test/pr/7\x1b\\#7 Subject\x1b]8;;\x1b\\"
    )


def test_converged_plan_discloses_metadata_updates(capsys):
    script = load_script()
    pr = pull_request(script, "a", 1, head_oid="a")
    stack = script.StackSnapshot("STACK_7", 7, [pr], {"a": pr})
    revisions = [revision(script, "a", local=("a",))]
    plan = script.make_plan(revisions, ["a"], stack, "base", (1,))
    repo = script.Repository(
        ".", "owner/repo", "owner/repo", "github.com", "main", "url"
    )
    ...
    output = capsys.readouterr().err
    assert "Plan: update stack #7" in output
    assert "Update title/body for #1." in output
    assert "No local, remote, or GitHub changes" not in output
...
def test_validates_complete_exact_old_suffix(monkeypatch):
    ...
    b = pull_request(script, "b", 2, base="a", base_oid="lagging-a", head_oid="b")
...
def test_first_live_segment_can_restack_onto_reported_pr_base(monkeypatch):
    ...
    script.verify_mechanical_restack(
        ...
        alternative_first_parent="current-main",
    )
...
def test_first_live_segment_accepts_parent_on_advancing_base(monkeypatch):
    script = load_script()
    pr = pull_request(
        script,
        "topic",
        1,
        head_oid="live-topic",
        base_oid="intermediate-main",
    )
    state = script.BoundaryState("topic", "live-topic", "live-topic", "live-topic", pr)
    monkeypatch.setattr(script, "ensure_git_commit", lambda *_args: None)
    monkeypatch.setattr(
        script,
        "git_linear_segment",
        lambda *_args: (_ for _ in ()).throw(script.Error("base advanced")),
    )
    monkeypatch.setattr(
        script, "git_last_linear_commits", lambda *_args: ("live-topic",)
    )
    monkeypatch.setattr(script, "git_commit_parent", lambda *_args: "old-main")
    ancestry = []
    monkeypatch.setattr(
        script,
        "git_is_ancestor",
        lambda root, ancestor, descendant: (
            ancestry.append((root, ancestor, descendant)) or True
        ),
    )

    assert script.verify_live_suffix(
        (state,),
        "current-main",
        (1,),
        "/git",
        "origin",
        allow_restacked_first=True,
        first_parent_ancestor_of="current-main",
    ) == (("live-topic",),)
    assert ancestry == [("/git", "old-main", "current-main")]

    monkeypatch.setattr(script, "git_is_ancestor", lambda *_args: False)
    with pytest.raises(script.Error, match="unexpected live base"):
        script.verify_live_suffix(
            (state,),
            "current-main",
            (1,),
            "/git",
...
def test_live_suffix_uses_commit_dag_instead_of_lagging_base_metadata(monkeypatch):
    script = load_script()
    a = pull_request(script, "a", 1, head_oid="live-a", base_oid="lagging-main")
    b = pull_request(script, "b", 2, head_oid="live-b", base="a", base_oid="lagging-a")
    states = (
        script.BoundaryState("a", "live-a", "live-a", "live-a", a),
        script.BoundaryState("b", "live-b", "live-b", "live-b", b),
    )
    monkeypatch.setattr(script, "ensure_git_commit", lambda *_args: None)
    edges = []
    monkeypatch.setattr(
        script,
        "git_linear_segment",
        lambda root, base, tip: edges.append((root, base, tip)) or (tip,),
    )

    assert script.verify_live_suffix(states, "live-main", (1, 1), "/git", "origin") == (
        ("live-a",),
        ("live-b",),
    )
    assert edges == [
        ("/git", "live-main", "live-a"),
        ("/git", "live-a", "live-b"),
    ]


def test_live_base_allows_only_monotonic_advances_when_unpinned(monkeypatch):
    script = load_script()
    monkeypatch.setattr(script, "ensure_git_commit", lambda *_args: None)
    ancestry = []
    monkeypatch.setattr(
        script,
        "git_is_ancestor",
        lambda root, old, new: ancestry.append((root, old, new)) or new == "new",
    )

    assert (
        script.verify_live_base(
            "/git", "origin", "main", "old", {"main": "new"}, pinned=False
        )
        == "new"
    )
    assert ancestry == [("/git", "old", "new")]
    with pytest.raises(script.Error, match="did not advance linearly"):
        script.verify_live_base(
            "/git", "origin", "main", "new", {"main": "rewound"}, pinned=False
        )
    with pytest.raises(script.Error, match="changed during reconciliation"):
        script.verify_live_base(
            "/git", "origin", "main", "old", {"main": "new"}, pinned=True
        )


def test_recovery_accepts_first_segment_restacked_onto_current_base(monkeypatch):
    script = load_script()
    pr = pull_request(
        script,
        "b",
    ...
    boundary = pull_request(
        ...
        state="MERGED",
        merged=True,
        head_oid="historical-a-oid",
    )
    stack = script.StackSnapshot("STACK_7", 7, [boundary], {"b": pr})
    evidence = script.RecoveryEvidence(
        pr.node_id,
        script.BaseTransition("EVENT_1", "now", "main", "historical-a"),
        boundary,
        "STACK_7",
        7,
    )
...
def test_boundary_validation_uses_git_fork_then_jj_history(monkeypatch):
    ...
    def revisions(revset):
        return {
            ...
            "(fork..tip) & divergent()": [],
            "(a | tip) & hidden()": [],
            "(a | tip) & stack_disallowed()": [],
        }[revset]

    monkeypatch.setattr(script, "revision_ids", revisions)
    monkeypatch.setattr(
        script,
        "parent_ids_for_commits",
        lambda commits: {"a": ("fork",), "tip": ("a",)},
    )
...
def test_stack_policy_checks_every_commit_in_publication_range(monkeypatch):
    script = load_script()
    calls = []

    def revisions(revset):
        calls.append(revset)
        if revset == "(boundary | interior | tip) & stack_disallowed()":
            return ["interior"]
        raise AssertionError(revset)

    outputs = {
        ("interior", 'change_id.shortest(8) ++ "\\n"'): "change12\n",
        ("interior", 'description.first_line() ++ "\\n"'): "fixup earlier work\n",
    }
    monkeypatch.setattr(script, "revision_ids", revisions)
    monkeypatch.setattr(
        script,
        "jj_output",
        lambda *_args: outputs[(_args[-3], _args[-1])],
    )

    with pytest.raises(script.Error, match="change12.*fixup earlier work") as error:
        script.validate_stack_commits(["boundary", "interior", "tip"])

    assert calls == ["(boundary | interior | tip) & stack_disallowed()"]
    assert "`jj describe`" in str(error.value)
    assert "`jj squash`" in str(error.value)


def test_stack_policy_accepts_publication_range_without_matches(monkeypatch):
    script = load_script()
    monkeypatch.setattr(script, "revision_ids", lambda _revset: [])

    script.validate_stack_commits(["boundary", "tip"])
...
def test_explicit_rebase_gives_local_authority_to_a_stale_converged_stack():
    script = load_script()
    current = script.Selection(("a",), ("a",), "base", "a")
    stale = script.Selection(("a",), ("a",), "old-base", "a")
    converged = script.ReconciliationKind.ALREADY_CONVERGED

    assert (
        script.apply_rebase_request(converged, stale, "base", requested=True)
        is script.ReconciliationKind.LOCAL_AUTHORITY
    )
    assert (
        script.apply_rebase_request(converged, current, "base", requested=True)
        is converged
    )
    assert (
        script.apply_rebase_request(converged, stale, "base", requested=False)
        is converged
    )
...
def test_final_verification_preserves_metadata_update_base_name(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: ({"c": current}, {"c": "oid-c"}),
    )
    monkeypatch.setattr(script, "fetch_stack", lambda *_args: final)
    monkeypatch.setattr(script, "verify_live_suffix", lambda *_args, **_kwargs: ())
    repo = script.Repository(
        ".",
        "owner/repo",
        "owner/repo",
...
def test_capture_closure_batches_change_ids(monkeypatch):
    script = load_script()
    calls = []
    monkeypatch.setattr(
        script,
        "jj_output",
        lambda *args: (
            calls.append(args) or "root-oid\troot-change\nchild-oid\tchild-change\n"
        ),
    )
    monkeypatch.setattr(
        script, "workspace_targets", lambda: (("default", "child-oid"),)
    )
    monkeypatch.setattr(
        script, "local_bookmark_targets", lambda: (("topic", "child-oid"),)
    )

    closure = script.capture_closure("root-oid")

    assert closure.commits == ("root-oid", "child-oid")
    assert closure.change_ids == ("root-change", "child-change")
    assert closure.workspaces == (("default", "child-oid"),)
    assert closure.bookmarks == (("topic", "child-oid"),)
    assert len(calls) == 1
    assert calls[0][calls[0].index("-r") + 1] == "root-oid::"
    assert calls[0][-1] == 'commit_id ++ "\\t" ++ change_id ++ "\\n"'


def test_identity_lookups_batch_and_reject_divergent_changes(monkeypatch):
    script = load_script()
    calls = []

    def output(*args):
        calls.append(args)
        if args[-1].startswith("commit_id"):
            return "oid-a\tchange-a\noid-b\tchange-b\n"
        return "change-a\toid-a-new\nchange-b\toid-b-new\n"

    monkeypatch.setattr(script, "jj_output", output)

    assert script.change_ids_for_commits(("oid-a", "oid-b")) == {
        "oid-a": "change-a",
        "oid-b": "change-b",
    }
    assert script.commit_ids_for_changes(("change-a", "change-b")) == {
        "change-a": "oid-a-new",
        "change-b": "oid-b-new",
    }
    assert len(calls) == 2
    ...
    monkeypatch.setattr(
        script,
        "jj_output",
        lambda *_args: "change-a\toid-a-one\nchange-a\toid-a-two\n",
    )
    with pytest.raises(script.Error, match="could not resolve rewritten jj change"):
        script.commit_ids_for_changes(("change-a",))


def test_rewritten_closure_allows_only_unselected_descendant_conflicts(
    monkeypatch, capsys
):
    script = load_script()
    before = script.ClosureSnapshot(
        "selected-old",
        ("selected-old", "sibling-old"),
        ("selected", "sibling"),
        frozenset(),
        (),
        (),
    ...
    monkeypatch.setattr(
        script,
        "commit_ids_for_changes",
        lambda change_ids: {change_id: f"{change_id}-new" for change_id in change_ids},
    )
    monkeypatch.setattr(script, "mutable_identities", frozenset)
    monkeypatch.setattr(script, "workspace_targets", tuple)
    monkeypatch.setattr(script, "local_bookmark_targets", tuple)
    conflicts = ["sibling-new"]
    monkeypatch.setattr(
        script,
        "revision_ids",
        lambda revset: conflicts if revset.endswith("& conflicts()") else [],
    )

    rewritten = script.verify_rewritten_closure(before, ("selected",))

    assert rewritten == {"selected": "selected-new", "sibling": "sibling-new"}
    warning = capsys.readouterr().err
    assert "Warning: 1 unselected descendant remains conflicted." in warning
    assert "sibling" not in warning

    script.VERBOSITY = 1
    script.verify_rewritten_closure(before, ("selected",))
    assert "Changes: sibling" in capsys.readouterr().err

    conflicts[:] = ["selected-new"]
    with pytest.raises(script.Error, match="selected stack commits"):
        script.verify_rewritten_closure(before, ("selected",))


def test_rewritten_closure_rejects_concurrent_mutable_changes(monkeypatch):
    script = load_script()
    before = script.ClosureSnapshot(
        "selected-old",
        ("selected-old",),
        ("selected",),
        frozenset({("selected", "selected-old"), ("concurrent", "concurrent-old")}),
        (),
        (),
    )
    monkeypatch.setattr(
        script,
        "commit_ids_for_changes",
        lambda _change_ids: {"selected": "selected-new"},
    )
    monkeypatch.setattr(
        script,
        "mutable_identities",
        lambda: frozenset(
            {("selected", "selected-new"), ("concurrent", "concurrent-new")}
        ),
    )

    with pytest.raises(script.Error, match="concurrent"):
        script.verify_rewritten_closure(before, ("selected",))
...
def test_refresh_closure_preserves_local_bookmarks_and_imports_base(monkeypatch):
    ...
    def revisions(revset):
        if revset == "new-main":
            return ["new-main"]
        if revset == "(stack | descendant) & visible()":
            return ["stack", "descendant"]
        raise AssertionError(revset)
    ...
    monkeypatch.setattr(
        ...
        "mutable_identities",
        lambda: frozenset(
            {("change-stack", "stack"), ("change-descendant", "descendant")}
        ),
    )

    refreshed = script.refresh_closure_baseline(before, "main", "new-main")

    assert refreshed.bookmarks == before.bookmarks
    assert refreshed.mutable is not None

    advanced_base = (("descendant", "descendant"), ("main", "new-main"))
    monkeypatch.setattr(script, "local_bookmark_targets", lambda: advanced_base)

    refreshed = script.refresh_closure_baseline(before, "main", "new-main")

    assert refreshed.bookmarks == advanced_base

    monkeypatch.setattr(
        script,
        "local_bookmark_targets",
        lambda: (("descendant", "descendant"), ("main", "unexpected")),
    )
    with pytest.raises(script.Error, match="changed a local bookmark"):
        script.refresh_closure_baseline(before, "main", "new-main")
    ...
    monkeypatch.setattr(
        script,
        "revision_ids",
        lambda revset: [] if revset == "new-main" else ["stack", "descendant"],
    )
...
def test_parser_requires_push_and_revset_and_rejects_abbreviations():
    ...
    assert script.parse_args(["push", "--rebase", "a"]).rebase
    with pytest.raises(SystemExit):
...
def test_existing_pr_cannot_move_to_a_different_change(monkeypatch):
    ...
    monkeypatch.setattr(
        script,
        "change_ids_for_commits",
        lambda oids: calls.append(tuple(oids)) or {"old": "change-old"},
    )
